...
- **Schema/metadata** — `XlogistxMongoMetaManager` (collection registration, `createIndex`, `nv_config_entities`) is left non-transactional. Avoids illegal in-transaction DDL and is idempotent. **First-use DDL inside a transaction is deferred to post-commit** (fix #57, `ensureCollectionDDL` / `flushDeferredDDL`): running `createIndex` out-of-band mid-transaction raced the transaction's implicit namespace creation and failed the commit with WriteConflict 112 "Collection namespace ... is already in use" (hit by `DMTool add-user` → `createSubjectID` on a fresh DB — the txn's snapshot is pinned by its first read, before the collection exists). Now the in-txn insert implicitly creates the collection (legal, Mongo 4.4+) and `endTransaction()` flushes the deferred index/registration work after a successful commit; on abort the deferred work is discarded. Consequence: during the very first transaction that uses a type, its unique indexes don't exist yet — pre-warming (insert one instance outside a txn) is no longer required for correctness but still guarantees unique-index enforcement from the first write.
