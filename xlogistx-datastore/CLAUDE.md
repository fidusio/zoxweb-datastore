...
| `createSubject_partialFailure_rollsBackAtomically()` | `H2PDomainSecurityManagerDBTest` | `createSubjectID` inserts subject + principal, then `createCredential` throws (non-`NVEntity` credential); asserts the whole unit of work rolled back — no subject, principal, or credential survives |
