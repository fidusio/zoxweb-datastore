...
//		SUBJECT_GUID(MetaToken.Subject_GUID),
	...
	{
		MongoMetaManager.SINGLETON.setMongoDatabase(connect());
	}
			...
			throws APIException
	{
		if (mongoDB == null)
		{
			...
			//		}
						...
						mongoDB = mongoClient.getDatabase(getAPIConfigInfo().getProperties().getValue(MongoDataStoreCreator.MongoParam.DB_NAME));
...
//		}
	...
	public MongoClient newConnection() throws APIException
	{
		try
		{
		    return  MongoClients.create((String) getAPIConfigInfo().getProperties().getValue(MongoDataStoreCreator.MongoParam.DB_URI));
		}
		catch(Exception e)
		{
			e.printStackTrace();
