...
//		realm.addRole(superAdminRole);
		...
		ShiroRole superAdminRole = Role.SUPER_ADMIN.toRole(domainID, appID);
		...
		for (SecurityModel.Permission permission : SecurityModel.Permission.values())
		{
			if (!exclusion.contains(permission))
			{
				...
				Role.addPermission(superAdminRole, permDAO);
			}
		}
		Role.addPermission(superAdminRole, SecurityModel.toPermission(domainID, appID, "assign_all", "Assign super admin mode", SecurityModel.PERM_ASSIGN_PERMISSION + ":*"));
		
		apiSecurityManager.addRole(superAdminRole);
		
		
		ShiroRole userRole = Role.USER.toRole(domainID, appID);
		...
		Role.addPermission(userRole, permDAO);
